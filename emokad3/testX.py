...
def Bandpass(data2, winsinc):
	[rowD,colD] = data2.shape
	#print data2.shape,winsinc.shape  
	if rowD < colD :
		data2 = np.transpose(data2)
		[rowD,colD] = data2.shape  
	banpassed = np.transpose(np.arange(rowD))
	#print data2.shape,winsinc.shape
	for j in range(data2.shape[1]):
		banpassed = np.vstack([banpassed,np.convolve((data2[:,j]),(winsinc), mode='same')])
		#print banpassed.shape
		
	return banpassed[1:rowD+1,:]
...
def cca(X,Y,t1,t2):
	...
	for i,Hz in enumerate(Y):
		#print i 
		[rowX,colX] = X.shape  
		[rowY,colY] = Y[Hz].shape
		if rowX > colX :
			X = np.transpose(X)
			[rowX,colX] = X.shape  
		if rowY > colY :
			Y[Hz] = np.transpose(Y[Hz])
			[rowY,colY] = Y[Hz].shape  
		#print X[:,t1:t2].shape, Y[Hz].shape
		#XY = np.asmatrix(np.hstack([data2[0:512,:],Ref]))  # [X;Y]
		XY = np.asmatrix(np.vstack([X[:,t1:t2],Y[Hz]]))  # [X;Y]
		...
		maxr = np.real(np.sqrt(r2 + (np.finfo(float).eps * 1j)))
		r.append(maxr)	
	return r
